...
- **Release workflow** — Add mypy stub for `configure_for_multiprocessing`; fix Windows Delta `path_to_table_uri` (extended `\\?\` paths produced invalid `file:///?/C:/...` URLs).
...
- **Release metadata** — Version bump to 4.6.0 across the Rust crates and the Python package.
...
- **Sparkless v4 (Python)** — Python package `sparkless` v4 in this repo: PySpark-like API backed by robin-sparkless (Rust). No JVM, no Polars Python at runtime. Install with `pip install ./python`. See [python/README.md](python/README.md).
