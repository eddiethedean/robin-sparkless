...
- **#235 – Type strictness: string vs numeric comparison** — Comparing a string column to a numeric literal in `filter` (e.g. `df.filter(col("str_col") == lit(123))`) no longer raises `RuntimeError: cannot compare string with numeric type (i32)`. The string side is coerced to a numeric type for equality and ordering comparisons in `DataFrame.filter`, matching PySpark-style coercion. New tests: `test_issue_235_type_strictness_comparisons.py`.
