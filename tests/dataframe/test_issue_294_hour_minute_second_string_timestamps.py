class TestIssue294HourMinuteSecondStringTimestamps:
    ...
    def _configure_spark_timezone(self, spark) -> None:
        """Ensure a deterministic session time zone for hour/minute/second behavior."""
        # Match the expectations in this test module: treat the input timestamps
        # as UTC and extract components in that time zone.
    ...
    def test_hour_minute_second_with_different_timezone_formats(self, spark):
        ...
        # Time zone offsets affect the extracted UTC hour in PySpark:
        # - +0000 / Z / no TZ (treated as session TZ=UTC): hour=4
        # - -0500: hour=9 (UTC)
        # - -0800: hour=12 (UTC)
        expected_hours = {
            "UTC": 4,
        ...
        for row in rows:
            name = row["Name"]
            assert row["hour"] == expected_hours[name], (
                f"Expected hour={expected_hours[name]} for {name}, got {row['hour']}"
            )
            assert row["minute"] == 0, (
                f"Expected minute=0 for {name}, got {row['minute']}"
            )
            assert row["second"] == 1, (
                f"Expected second=1 for {name}, got {row['second']}"
            )
