...
def test_filter_col_gt_col_pyspark_parity() -> None:
    ...
    actual = df.filter(F.col("a") > F.col("b")).collect()
...
def test_filter_col_eq_col_pyspark_parity() -> None:
    ...
    schema = ["x", "y"]
    df = spark.createDataFrame(data, schema)
    actual = df.filter(F.col("x") == F.col("y")).collect()
...
def test_filter_combined_col_col_and_literal_pyspark_parity() -> None:
    ...
    schema = ["a", "b"]
    df = spark.createDataFrame(data, schema)
    actual = df.filter((F.col("a") > F.col("b")) & (F.col("a") > 2)).collect()
...
def test_with_column_col_gt_col_pyspark_parity() -> None:
    ...
    schema = ["p", "q"]
    df = spark.createDataFrame(data, schema)
    actual = df.withColumn("p_gt_q", F.col("p") > F.col("q")).collect()
...
def test_filter_col_gt_col_strings_pyspark_parity() -> None:
    ...
    schema = ["s1", "s2"]
    df = spark.createDataFrame(data, schema)
    actual = df.filter(F.col("s1") > F.col("s2")).collect()
