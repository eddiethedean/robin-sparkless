"""Tests for F.create_map() function. Expectations match PySpark behavior."""
...
class TestCreateMap:
    ...
    @pytest.mark.skip(
        reason="Issue #1140: unskip when fixing create_map collected value type (string vs int)"
    )
    def test_create_map_with_literals(self, spark):
        ...
        assert len(rows) == 1
        # PySpark returns map values from columns as strings when collected to Python
    ...
    def test_create_map_validation_odd_args(self, spark):
        """Test create_map with odd number of arguments raises when evaluated (PySpark behavior)."""
        df = spark.createDataFrame([{"x": 1}])
        with pytest.raises(Exception):
            df.select(F.create_map(F.lit("key1")).alias("m")).collect()
    ...
    def test_create_map_in_withcolumn(self, spark):
        """Test create_map works with withColumn. PySpark returns map values as strings when collected."""
        ...
        assert rows[0]["info"]["age"] in (25, "25")  # PySpark often returns string "25"
    ...
    def test_create_map_multiple_maps_in_select(self, spark):
        """Test creating multiple maps in a single select. PySpark may return numeric map values as strings."""
        df = spark.createDataFrame([{"name": "Alice", "age": 25}])
        result = df.select(
        ...
        assert rows[0]["age_map"]["age"] in (25, "25")
    ...
    # Additional robust tests for Issue #365 - empty map via create_map() (PySpark accepts no-arg or [])
    def test_create_map_empty_tuple_returns_empty_map(self, spark):
        """Test create_map() with no arguments returns empty map {} (PySpark behavior)."""
        df = spark.createDataFrame([{"Name": "Alice"}, {"Name": "Bob"}])
        result = df.withColumn("NewMap", F.create_map())
        rows = result.collect()
        assert len(rows) == 2
