...
Charlie,35,70000.75,true"""
    ...
    def test_csv_explicit_infer_schema_true(self, spark, sample_csv):
        ...
        # age: PySpark infers int as IntegerType or LongType
        assert isinstance(field_dict["age"], (IntegerType, LongType)), (
            "age should be IntegerType or LongType"
        )
...
002,456"""
        ...
        try:
            ...
            assert isinstance(field_dict_inferred["id"], (IntegerType, LongType)), (
                "id should be LongType"
            )
            assert isinstance(field_dict_inferred["value"], (IntegerType, LongType)), (
...
003,0003"""
        ...
        try:
            ...
            assert isinstance(field_dict_inferred["code"], (IntegerType, LongType)), (
...
-999999999999,-999999999999.999"""
        ...
        try:
            ...
            assert isinstance(field_dict["big_int"], (IntegerType, LongType)), (
...
Alice,25,95.5"""
    ...
    def test_create_dataframe_empty_list(self, spark):
        ...
        # Empty list without schema should raise (PySpark: message contains "empty dataset")
        with pytest.raises(Exception, match="empty dataset"):
    ...
    def test_create_dataframe_type_conflict_raises_error(self, spark):
        """Test type conflicts: PySpark may raise or infer string."""
        data = [
            {"value": 1},  # int
        ...
        try:
            df = spark.createDataFrame(data)
            # If no exception, PySpark inferred a type (e.g. string); verify we have value column
            assert "value" in df.columns
            assert len(df.schema.fields) >= 1
        except (TypeError, ValueError) as e:
            assert "merge" in str(e).lower() or "type" in str(e).lower()
...
100"""
        ...
        try:
            ...
            assert isinstance(
                df_inferred.schema.fields[0].dataType, (IntegerType, LongType)
            ), "Should be IntegerType or LongType"
        finally:
            Path(temp_path).unlink(missing_ok=True)
...
Bob,"Single line"'''
        ...
        try:
            ...
            # PySpark may parse quoted newlines as row boundaries (2 or 3 rows)
            assert df.count() >= 2, "Should have at least 2 rows"
            for field in df.schema.fields:
                assert isinstance(field.dataType, StringType), (
                    f"{field.name} should be StringType"
                )
