...
- **Sparkless backend target**: Intended to power Sparkless's execution engine; aligns with its 403+ PySpark functions and 270+ test fixtures; **159 parity fixtures** passing (~295+ functions), plus **3 plan fixtures** (filter_select_limit, join_simple, with_column_functions) for the plan interpreter; **expression interpreter** supports all scalar functions in filter/select/withColumn (see [LOGICAL_PLAN_FORMAT.md](docs/LOGICAL_PLAN_FORMAT.md)). **Gap closure (Feb 2026)**: Bitmap (5), make_dt_interval, make_ym_interval, to_timestamp_ltz/ntz, sequence, shuffle, inline, inline_outer, regr_* (9); DataFrame cube, rollup, write (parquet/csv/json), data, toLocalIterator, persist/unpersist, and stubs (rdd, foreach, mapInPandas, etc.). **Signature alignment** (Feb 2026): optional params and two-arg when. **Phase 25 completed**: Plan interpreter (`execute_plan`), expression interpreter, logical plan schema, `create_dataframe_from_rows`. **Phase 24 completed**: bit, control, JVM stubs, rand/randn, AES crypto. **Phase 23 completed**: JSON/URL/misc. **Phase 22 completed**: datetime extensions. **Phase 21 completed**: string, binary, type, array/map/struct. Phases 20–18: ordering, aggregates, array/map/struct. Deferred: XML, XPath, sentences (see [docs/ROBIN_SPARKLESS_MISSING.md](docs/ROBIN_SPARKLESS_MISSING.md)). Path to 100%: Phase 26 (publish crate), Phase 27 (Sparkless integration).
...
- [docs/PARITY_STATUS.md](docs/PARITY_STATUS.md) – PySpark parity coverage matrix (159 fixtures)
