...
- **Sparkless backend target**: Intended to power Sparkless's execution engine; aligns with its 403+ PySpark functions and 270+ test fixtures; **159 parity fixtures** passing (~283+ functions), plus **3 plan fixtures** (filter_select_limit, join_simple, with_column_functions) for the plan interpreter; **expression interpreter** supports all scalar functions in filter/select/withColumn (see [LOGICAL_PLAN_FORMAT.md](docs/LOGICAL_PLAN_FORMAT.md)). **Signature alignment** (Feb 2026): optional params implemented for assert_true(errMsg), like/ilike(escapeChar), months_between(roundOff), parse_url(key), make_timestamp(timezone), to_char/to_timestamp/to_varchar/try_to_timestamp(format), position(start); two-arg when(condition, value); PyO3 signatures and into_py fixes. **Phase 25 completed**: Plan interpreter (`execute_plan`), expression interpreter (serialized expr → Expr), logical plan schema ([LOGICAL_PLAN_FORMAT.md](docs/LOGICAL_PLAN_FORMAT.md)), `create_dataframe_from_rows` (Rust + Python). **Phase 24 completed**: bit, control, JVM stubs, rand/randn, AES crypto (aes_encrypt, aes_decrypt, try_aes_decrypt). **Phase 23 completed**: JSON/URL/misc. **Phase 22 completed**: datetime extensions. **Phase 21 completed**: string, binary, type, array/map/struct. Phases 20–18: ordering, aggregates, array/map/struct. Path to 100%: Phase 26 (publish crate), Phase 27 (Sparkless integration).
