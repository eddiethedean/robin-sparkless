...
impl Column {
    ...
    /// string column. PySpark, however, coerces types and performs the comparison.
    ///
    /// This helper mirrors that behavior for Python-facing comparisons by:
    /// - Detecting `Column <op> numeric literal` patterns.
    /// - Casting the column side to `Float64` before applying the comparison.
    pub fn eq_with_numeric_literal(&self, other: &Column) -> Column {
        ...
        let right = other.expr().clone();

        if std::env::var("ROBIN_DEBUG_235").is_ok() {
            eprintln!("ROBIN_DEBUG_235 eq_with_numeric_literal left={left:?} right={right:?}");
        }

        let expr = if is_numeric_literal_expr(&right) {
            left.cast(DataType::Float64).eq(right)
        } else if is_numeric_literal_expr(&left) {
            left.eq(right.cast(DataType::Float64))
        } else {
            left.eq(right)
        };
    ...
    /// Inequality comparison with string-vs-numeric literal handling.
    pub fn neq_with_numeric_literal(&self, other: &Column) -> Column {
        use polars::prelude::{DataType, LiteralValue};
        ...
        let expr = if is_numeric_literal_expr(&right) {
            left.cast(DataType::Float64).neq(right)
        } else if is_numeric_literal_expr(&left) {
            left.neq(right.cast(DataType::Float64))
        } else {
            left.neq(right)
        };
    ...
    /// Greater-than comparison with string-vs-numeric literal handling (issue #235).
    pub fn gt_with_numeric_literal(&self, other: &Column) -> Column {
        use polars::prelude::{DataType, LiteralValue};
        ...
        let expr = if is_numeric_literal_expr(&right) {
            left.cast(DataType::Float64).gt(right)
        } else if is_numeric_literal_expr(&left) {
            left.gt(right.cast(DataType::Float64))
        } else {
            left.gt(right)
        };
    ...
    /// Greater-than-or-equal comparison with string-vs-numeric literal handling.
    pub fn ge_with_numeric_literal(&self, other: &Column) -> Column {
        use polars::prelude::{DataType, LiteralValue};
        ...
        let expr = if is_numeric_literal_expr(&right) {
            left.cast(DataType::Float64).gt_eq(right)
        } else if is_numeric_literal_expr(&left) {
            left.gt_eq(right.cast(DataType::Float64))
        } else {
            left.gt_eq(right)
        };
    ...
    /// Less-than comparison with string-vs-numeric literal handling.
    pub fn lt_with_numeric_literal(&self, other: &Column) -> Column {
        use polars::prelude::{DataType, LiteralValue};
        ...
        let expr = if is_numeric_literal_expr(&right) {
            left.cast(DataType::Float64).lt(right)
        } else if is_numeric_literal_expr(&left) {
            left.lt(right.cast(DataType::Float64))
        } else {
            left.lt(right)
        };
    ...
    /// Less-than-or-equal comparison with string-vs-numeric literal handling.
    pub fn le_with_numeric_literal(&self, other: &Column) -> Column {
        use polars::prelude::{DataType, LiteralValue};
        ...
        let expr = if is_numeric_literal_expr(&right) {
            left.cast(DataType::Float64).lt_eq(right)
        } else if is_numeric_literal_expr(&left) {
            left.lt_eq(right.cast(DataType::Float64))
        } else {
            left.lt_eq(right)
        };
