...
PYSPARK_TYPING_NAMES = frozenset({
    ...
    "ValuesView", "AbstractSet", "ByteString", "Container", "ContextManager", "Counter",
    "DefaultDict", "Deque", "FrozenSet", "Generator", "Generic", "Hashable", "ItemsView",
    "KeysView", "Mapping", "MappingView", "MutableMapping", "MutableSet", "Sequence",
    "Set", "Sized", "TYPE_CHECKING", "TypeVar", "cast", "overload", "no_type_check",
    "get_args", "get_origin", "get_type_hints", "ParamSpec", "Concatenate", "TypedDict",
    "NamedTuple", "Protocol", "runtime_checkable", "SupportsAbs", "SupportsBytes",
...
def compare_class_methods(py_data: dict, robin_data: dict, class_name: str) -> list[dict]:
    py_methods = {m["name"]: m for m in (py_data.get("classes") or {}).get(class_name) or []}
    robin_methods = {m["name"]: m for m in (robin_data.get("classes") or {}).get(class_name) or []}
...
def write_gap_analysis_md(out: dict, path: Path) -> None:
    ...
    lines = [
        ...
        "|----------------|-------|-------------|",
        "| exact | " + str(summary.get("functions", {}).get("exact", 0)) + " | Same parameter names, order, and defaults |",
        "| compatible | " + str(summary.get("functions", {}).get("compatible", 0)) + " | Same params/defaults; types may differ |",
        "| partial | " + str(summary.get("functions", {}).get("partial", 0)) + " | Different param names or counts (e.g. `column` vs `col`) |",
        "| missing | " + str(summary.get("functions", {}).get("missing", 0)) + " | In PySpark but not in robin-sparkless |",
        "| extra | " + str(summary.get("functions", {}).get("extra", 0)) + " | In robin-sparkless but not in PySpark (extensions) |",
        "",
    ...
    for cls_name in ["SparkSession", "SparkSessionBuilder", "DataFrame", "GroupedData", "Column", "DataFrameStat", "DataFrameNa"]:
    ...
    if len(partial_funcs) > 25:
        lines.append(f"| *... and {len(partial_funcs) - 25} more (see signature_comparison.json)* | |")
    lines.extend([
        "",
    ...
    if len(missing_funcs) > 40:
        lines.append(f"- ... and {len(missing_funcs) - 40} more (see `signature_comparison.json`)")
    lines.extend([
        "",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Compare PySpark and robin_sparkless signatures")
    parser.add_argument("--pyspark", default="docs/signatures_pyspark.json", help="PySpark signatures JSON")
    parser.add_argument("--robin", default="docs/signatures_robin_sparkless.json", help="Robin signatures JSON")
    parser.add_argument("--output", default="docs/signature_comparison.json", help="Output comparison JSON")
    parser.add_argument("--md", action="store_true", help="Print markdown summary to stdout")
    parser.add_argument("--write-md", metavar="PATH", help="Write full SIGNATURE_GAP_ANALYSIS.md to PATH")
    ...
    classes = ["SparkSession", "SparkSessionBuilder", "DataFrame", "GroupedData", "Column", "DataFrameStat", "DataFrameNa"]
    class_comparisons = {c: compare_class_methods(py_data, robin_data, c) for c in classes}
    ...
    if args.md:
        ...
        for k, v in counts.items():
            print(f"| {k} | {v} |")
        print(f"\nPySpark functions (excluding typing): {out['summary']['function_total_pyspark']}")
        print(f"Robin-sparkless functions: {out['summary']['function_total_robin']}")
