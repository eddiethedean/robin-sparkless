...
def _more_scenarios() -> list[Scenario]:
    ...
    string_cases: list[tuple[str, str, Callable[[Any], Any]]] = [
        (
            "string.upper",
            "String: upper()",
            lambda session: session.createDataFrame([("Abc",), ("",), (None,)], ["s"])
        ...
        (
            "string.lower",
            "String: lower()",
            lambda session: session.createDataFrame([("AbC",), ("XYZ",), (None,)], ["s"])
        ...
        (
            "string.length",
            "String: length()",
            lambda session: session.createDataFrame([("a",), ("ab",), ("",), (None,)], ["s"])
        ...
        (
            "string.trim",
            "String: trim()",
            lambda session: session.createDataFrame([("  a  ",), ("\txy\n",), (None,)], ["s"])
        ...
        (
            "string.substring",
            "String: substring()",
            lambda session: session.createDataFrame([("abcdef",), ("ab",), (None,)], ["s"])
        ...
        (
            "string.concat_ws",
            "String: concat_ws()",
            lambda session: session.createDataFrame([("a", "b"), ("a", None), (None, "c")], ["a", "b"])
            .select(F(session).concat_ws("-", F(session).col("a"), F(session).col("b")).alias("out"))
            .orderBy("out"),
        ),
        (
            "string.regexp_replace",
            "String: regexp_replace()",
            lambda session: session.createDataFrame([("a1b2",), ("",), (None,)], ["s"])
        ...
        (
            ...
            "String: regexp_extract()",
            lambda session: session.createDataFrame([("abc123",), ("zzz",), (None,)], ["s"])
        ...
        (
            "string.split_basic",
            "String: split() basic",
            lambda session: session.createDataFrame([("a,b,c",), ("a",), (None,)], ["s"])
    ...
    numeric_cases: list[tuple[str, str, Callable[[Any], Any]]] = [
        (
            "numeric.abs",
            "Numeric: abs()",
            lambda session: session.createDataFrame([(-3,), (0,), (2,), (None,)], ["x"])
        ...
        (
            "numeric.round",
            "Numeric: round()",
            lambda session: session.createDataFrame([(1.2345,), (1.2,), (None,)], ["x"])
        ...
        (
            "numeric.floor",
            "Numeric: floor()",
            lambda session: session.createDataFrame([(1.9,), (-1.1,), (None,)], ["x"])
        ...
        (
            "numeric.ceil",
            "Numeric: ceil()",
            lambda session: session.createDataFrame([(1.1,), (-1.9,), (None,)], ["x"])
        ...
        (
            "numeric.greatest",
            "Numeric: greatest()",
            lambda session: session.createDataFrame([(1, 2), (None, 3), (5, None)], ["a", "b"])
        ...
        (
            "numeric.sqrt",
            "Numeric: sqrt()",
            lambda session: session.createDataFrame([(4.0,), (2.0,), (0.0,), (None,)], ["x"])
        ...
        (
            "numeric.pmod",
            "Numeric: pmod()",
            lambda session: session.createDataFrame([(-5, 3), (5, 3), (None, 3)], ["a", "b"])
        ...
        (
            "numeric.signum",
            "Numeric: signum()",
            lambda session: session.createDataFrame([(-5,), (0,), (5,), (None,)], ["x"])
        ...
        (
            "numeric.log10",
            "Numeric: log10()",
            lambda session: session.createDataFrame([(1.0,), (10.0,), (100.0,), (None,)], ["x"])
    ...
    date_cases: list[tuple[str, str, Callable[[Any], Any]]] = [
        (
            "date.to_date",
            "Date: to_date()",
            lambda session: session.createDataFrame([("2020-01-02",), ("invalid",), (None,)], ["s"])
        ...
        (
            "date.date_add",
            "Date: date_add()",
            lambda session: session.createDataFrame([("2020-01-02",), (None,)], ["s"])
        ...
        (
            "date.month",
            "Date: month()",
            lambda session: session.createDataFrame([("2020-12-02",), (None,)], ["s"])
        ...
        (
            "date.datediff",
            "Date: datediff()",
            lambda session: session.createDataFrame([("2020-01-10", "2020-01-02"), (None, "2020-01-02")], ["a", "b"])
            .select(
                F(session).datediff(F(session).to_date("a"), F(session).to_date("b")).alias("dd")
            )
            .orderBy("dd"),
        ),
        (
            "date.add_months",
            "Date: add_months()",
            lambda session: session.createDataFrame([("2020-01-31",), ("2020-02-29",), (None,)], ["s"])
        ...
        (
            ...
            "Date: date_format()",
            lambda session: session.createDataFrame([("2020-01-02",), (None,)], ["s"])
            .select(F(session).date_format(F(session).to_date("s"), "yyyy-MM-dd").alias("out"))
            .orderBy("out"),
        ),
        (
            "date.make_date",
            "Date: make_date()",
            lambda session: session.createDataFrame([(2020, 1, 2), (2020, 2, 30), (None, 1, 2)], ["y", "m", "d"])
    ...
    array_cases: list[tuple[str, str, Callable[[Any], Any]]] = [
        (
            "array.size",
            "Array: size()",
            lambda session: session.createDataFrame([(["a", "b"],), ([],), (None,)], ["arr"])
        ...
        (
            ...
            lambda session: session.createDataFrame([(["a", "b"],), (["x"],), (None,)], ["arr"])
            .select(F(session).array_contains(F(session).col("arr"), "a").alias("out"))
            .orderBy("out"),
        ),
        (
            "array.element_at",
        ...
        (
            "array.concat",
            "Array: concat()",
            lambda session: session.createDataFrame([(["a"], ["b"]), ([], ["x"]), (None, ["y"])], ["a", "b"])
            .select(F(session).concat(F(session).col("a"), F(session).col("b")).alias("out"))
            .orderBy("out"),
        ),
        (
            "array.sort_array",
            "Array: sort_array()",
            lambda session: session.createDataFrame([(["b", "a"],), (["a", "a"],), (None,)], ["arr"])
        ...
        (
            ...
            lambda session: session.createDataFrame([(["a", "a", "b"],), ([],), (None,)], ["arr"])
        ...
        (
            "array.slice",
            "Array: slice()",
            lambda session: session.createDataFrame([(["a", "b", "c"],), (["x"],), (None,)], ["arr"])
        ...
        (
            ...
            lambda session: session.range(1, 2).select(
                F(session)
                .array(F(session).struct(F(session).lit(1).alias("x"), F(session).lit("a").alias("s")))
                .alias("arr")
            ),
        ),
    ]
    ...
    df_cases: list[tuple[str, str, Callable[[Any], Any]]] = [
        (
            ...
            lambda session: session.createDataFrame([(1,), (2,), (None,)], ["x"])
        ...
        (
            ...
            lambda session: session.createDataFrame([(1,)], ["x"]).withColumnRenamed("x", "y"),
        ),
        (
            "df.filter_isnull",
            "DataFrame: filter(isNull)",
            lambda session: session.createDataFrame([(1,), (None,), (2,)], ["x"])
        ...
        (
            "df.distinct",
            "DataFrame: distinct()",
            lambda session: session.createDataFrame([(1,), (1,), (2,)], ["x"]).distinct().orderBy("x"),
        ),
        (
            "df.dropduplicates",
            "DataFrame: dropDuplicates()",
            lambda session: session.createDataFrame([(1, "a"), (1, "a"), (1, "b")], ["x", "y"])
            .dropDuplicates(["x", "y"])
            .orderBy("x", "y"),
        ),
        (
            ...
            lambda session: session.createDataFrame([(1, "a")], ["x", "y"])
        ...
        (
            ...
            lambda session: session.createDataFrame([(1, 2)], ["a", "b"]).selectExpr("a + b as c"),
        ),
        (
            "df.na_fill",
            "DataFrame: na.fill()",
            lambda session: session.createDataFrame([(None,), (1,), (None,)], ["x"]).na.fill(0).orderBy("x"),
        ),
        (
            "df.limit",
            "DataFrame: limit()",
        ...
        (
            ...
            lambda session: session.createDataFrame([(1, "b"), (1, "a"), (2, "a")], ["x", "y"]).orderBy("x", "y"),
        ),
    ]
    for sid, title, fn in df_cases:
        out.append(Scenario(id=sid, title=title, fn=fn, tags=["dataframe", "generated"]))
    ...
    for how in ["inner", "left", "right", "full", "left_semi", "left_anti"]:
        ...
        def _mk_join(h: str) -> Callable[[Any], Any]:
            def f(session):
                left = session.createDataFrame([(1, "a"), (2, "b"), (None, "n")], ["id", "v"])
                right = session.createDataFrame([(1, "x"), (3, "y"), (None, "z")], ["id", "w"])
    ...
    win_cases: list[tuple[str, str, Callable[[Any], Any]]] = [
        ...
        (
            "window.lag",
            "Window: lag()",
            lambda session: session.createDataFrame([(1, "a", 10), (2, "a", 20), (3, "a", 30)], ["id", "grp", "x"])
            .withColumn(
                "prev",
                F(session).lag("x", 1).over(Window(session).partitionBy("grp").orderBy("id")),
            )
            .orderBy("id"),
        ),
        (
            ...
            .withColumn(
                "nxt",
                F(session).lead("x", 1).over(Window(session).partitionBy("grp").orderBy("id")),
            )
            .orderBy("id"),
        ),
        (
            ...
            lambda session: session.createDataFrame([(1, "a", 10), (2, "a", 20), (3, "b", 30)], ["id", "grp", "x"])
            .withColumn("s", F(session).sum("x").over(Window(session).partitionBy("grp")))
            .orderBy("id"),
        ),
        (
            "window.avg_over",
            ...
            lambda session: session.createDataFrame([(1, "a", 10), (2, "a", 20), (3, "b", 30)], ["id", "grp", "x"])
            .withColumn("a", F(session).avg("x").over(Window(session).partitionBy("grp")))
            .orderBy("id"),
        ),
    ]
    ...
    agg_cases: list[tuple[str, str, Callable[[Any], Any]]] = [
        (
            "agg.groupby_sum",
            "Agg: groupBy().sum()",
            lambda session: session.createDataFrame([("a", 1), ("a", 2), ("b", 3), (None, 4)], ["g", "x"])
            .groupBy("g")
            .sum("x")
            .orderBy("g"),
        ),
        (
            "agg.groupby_count",
            "Agg: groupBy().count()",
            lambda session: session.createDataFrame([("a",), ("a",), ("b",), (None,)], ["g"])
            .groupBy("g")
            .count()
            .orderBy("g"),
        ),
        ...
        (
            ...
            lambda session: session.createDataFrame([("a", 1), ("a", 2), ("b", 3)], ["g", "x"])
            .groupBy("g")
    ...
    def _sql_group_by(session):
        session.createDataFrame([("a", 1), ("a", 2), ("b", 3)], ["g", "x"]).createOrReplaceTempView(
            "t2"
        )
