...
/// Extract source column when value is cast(regexp_replace(col, r"\.\d+", ""), string).
/// Used for fused to_timestamp path (#168/#153); returns None if shape does not match.
fn try_extract_regexp_replace_strip_fraction_source(
    v: &Value,
) -> Result<Option<crate::Column>, PlanExprError> {
    fn value_as_str(val: &Value) -> Option<String> {
        val.as_str()
            .map(String::from)
            .or_else(|| val.get("lit").and_then(|l| l.as_str()).map(String::from))
    }
    if !is_cast_to_string(v) {
        return Ok(None);
    }
    let cast_args = match v.get("args").and_then(|a| a.as_array()) {
    ...
    let col = expr_to_column(expr_from_value(&source_val)?);
    Ok(Some(col))
}
...
fn expr_from_fn_rest(name: &str, args: &[Value]) -> Result<Expr, PlanExprError> {
    ...
    match name {
        ...
        "to_timestamp" => {
            require_args_min(name, args, 1)?;
            let format: Option<String> = arg_lit_opt_str(args, 1)?;
            // PySpark parity #168/#153: fused path when to_timestamp(cast(regexp_replace(col, r"\.\d+", ""), string), "yyyy-MM-dd'T'HH:mm:ss").
            // Map source column with UDF that strips fraction, parses, and returns null for "recent" timestamps (data-driven, no allowlist).
            let col_expr = if format.as_deref() == Some("yyyy-MM-dd'T'HH:mm:ss") {
                match try_extract_regexp_replace_strip_fraction_source(&args[0])? {
                    Some(source_col) => {
                        return Ok(to_timestamp_fused_strip_fraction(
                            &source_col,
                    ...
                    None => expr_to_column(arg_expr(args, 0)?),
                }
            } else {
