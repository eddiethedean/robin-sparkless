...
- **Unsupported (tracked in #141)**: DDL (`CREATE/DROP DATABASE`, `CREATE/DROP TABLE`, `CREATE SCHEMA`, `SET CURRENT DATABASE`, etc.), DML (`INSERT INTO`), subqueries in `FROM`, CTEs. **Supported:** `UPDATE table SET col = expr [WHERE condition]` and `DELETE FROM table [WHERE condition]` (single table; table must exist in session catalog from saveAsTable or temp view).
