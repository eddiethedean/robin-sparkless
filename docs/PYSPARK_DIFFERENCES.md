...
- **UDF / UDTF (#143)**: Scalar UDFs **implemented**: `spark.udf().register()`, `call_udf`, Rust `register_udf`. Python UDFs run row-at-a-time (eager); `pandas_udf` (vectorized) and `udtf` deferred. Python UDF in WHERE/HAVING not yet supported. See [UDF_GUIDE.md](UDF_GUIDE.md).
