...
```toml
[dependencies]
robin-sparkless = "0.11.10"
```

Optional features:

```toml
robin-sparkless = { version = "0.11.10", features = ["sql"] }   # spark.sql(), temp views
robin-sparkless = { version = "0.11.10", features = ["delta"] }  # Delta Lake read/write
```
