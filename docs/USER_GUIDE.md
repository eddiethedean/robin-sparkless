...
```toml
[dependencies]
robin-sparkless = "0.9.1"
```

Optional features:

```toml
robin-sparkless = { version = "0.9.1", features = ["sql"] }   # spark.sql(), temp views
robin-sparkless = { version = "0.9.1", features = ["delta"] }  # Delta Lake read/write
```
