...
```toml
[dependencies]
robin-sparkless = "0.6.0"
```

Optional features:

```toml
robin-sparkless = { version = "0.6.0", features = ["sql"] }   # spark.sql(), temp views
robin-sparkless = { version = "0.6.0", features = ["delta"] }  # Delta Lake read/write
```
