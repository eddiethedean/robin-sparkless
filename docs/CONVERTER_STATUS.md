...
- **Target**: Convert Sparkless `expected_outputs` with `--batch` and `--output-subdir converted`; run `make sparkless-parity` (set `SPARKLESS_EXPECTED_OUTPUTS` when Sparkless repo is available). Goal: 50+ tests passing (hand-written + converted). Current: 103 hand-written fixtures passing (Phase 17: unix_timestamp, from_unixtime, make_date, timestamp_*, unix_date, date_from_unix_date, pmod, factorial; Phase 16: regexp_count, regexp_instr, regexp_substr, split_part, find_in_set, format_string, printf; Phase 15: aliases, string, math, array_distinct; array_distinct skipped). Target: 150+ total fixtures with converted outputs.
