...
- [ ] **Metadata gate** — Run `python3 scripts/check_release_versions.py --tag vX.Y.Z` and `python3 -m unittest discover -s scripts -p test_check_release_versions.py`. Use Python 3.11+ (or install tomli for Python 3.8–3.10). `spark-sql-parser` has its own version and need not change when its sources are unchanged.
