...
3. **CI runs automatically** on the tag push (see [.github/workflows/release.yml](../.github/workflows/release.yml)):
   ...
   - Then the workflow publishes to crates.io **in dependency order**:
     ...
     3. `robin-sparkless-expr`
     4. `robin-sparkless`

4. **Verify**: Check [crates.io/crates/robin-sparkless](https://crates.io/crates/robin-sparkless), [robin-sparkless-core](https://crates.io/crates/robin-sparkless-core), [robin-sparkless-expr](https://crates.io/crates/robin-sparkless-expr), [spark-sql-parser](https://crates.io/crates/spark-sql-parser), and their docs on docs.rs after the workflow completes.
...
- The three robin-sparkless crates are published with the same version number so that the main crate can depend on `robin-sparkless-core = "0.12"` and `robin-sparkless-expr = "0.12"` and resolve to the matching release. `spark-sql-parser` may use a separate version (e.g. `0.1.x`).
