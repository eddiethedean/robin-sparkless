...
- Tags must match the version in the root `Cargo.toml` (e.g. tag `v0.17.0` only when root and both crates have `version = "0.17.0"`).
- Do not re-tag or overwrite tags; crates.io does not allow republishing the same version.
- The three robin-sparkless crates are published with the same version number so that the main crate can depend on `robin-sparkless-core = "0.17"` and `robin-sparkless-polars = "0.17"` and resolve to the matching release. `spark-sql-parser` may use a separate version (e.g. `0.2.x`).
